...
// `C.binTo64('111000')` returns '4'
C.binTo64 = function(input) {
  if ( typeof input != 'string' || input.match(/[^01]/) )
    return console.error('invalid arg on binTo64: binary strings only ')
  //check if leading zeroes are necessary
...
C.hexTo64 = function(input) {
  if ( typeof input != 'string' || input.match(/[^a-fA-F0-9]/) )
    return console.error('invalid arg on hextTo64: hexadecimal strings only')
...
C.asciiTo64 = function(input) {
  if ( typeof input != 'string' )
    return console.error('invalid arg on asciiTo64: ascii strings only')
